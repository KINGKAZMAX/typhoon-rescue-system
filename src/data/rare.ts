...
import { Syringe, Stethoscope, Activity, Pill, Ribbon, Milk } from 'lucide-react'
...
export const emergencyMeds: EmergencyMed[] = [
  ...
  },
  {
    id: 'metabolic',
    icon: Milk,
    condition: '遗传代谢病 / 需特殊医学配方食品（特食，如苯丙酮尿症）',
    guidance:
      '这类患者靠特殊医学用途配方食品（特食：无苯丙氨酸奶粉/专用低蛋白食品等）维持代谢稳定，断供或误食普通高蛋白食物可能在数天内诱发代谢危象。灾时第一要务是保住现有特食并密封防潮防水、清点还能吃几天；尽快联系原就诊代谢/营养科，或走病痛挑战、蔻德、OpenRD 等组织协调调配转运。短期断供先严格限制天然蛋白摄入、补足无蛋白热量（如葡萄糖水/专用低蛋白主食）保证不饿着，具体食谱与配比一律以代谢科医生为准。',
    danger: '嗜睡、拒食、反复呕吐、呼吸深快、意识改变或抽搐（代谢危象）→ 立即拨 120，并说明是遗传代谢病/需特食患者。',
  },
]

// ── "我的应急包"通用清单 ──
export const emergencyKit: string[] = [
  '想清楚"最需要什么疾病相关支持"：药物、医疗处置（透析/供氧/呼吸机）、还是特殊医学配方食品（特食）——一句话说清最救命',
  '备用药量：至少 3–7 天用量 + 处方/病历副本（可拍照存手机）',
  '特食储备：依赖特食者备足并密封防水，清点还能吃几天，尽早联系代谢/营养科补给',
